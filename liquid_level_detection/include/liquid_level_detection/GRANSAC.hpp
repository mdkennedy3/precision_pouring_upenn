...
#include <liquid_level_detection/AbstractModel.hpp>
...
namespace GRANSAC
{
	...
	private:
		...
		std::vector<std::shared_ptr<AbstractParameter>> m_BestInliers, m_BestOutliers;
	...
	public:
		...
		const std::vector<std::shared_ptr<AbstractParameter>>& GetBestOutliers(void) { return m_BestOutliers; };
		...
		bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>> &Data)
		{
			...
			std::vector<std::vector<std::shared_ptr<AbstractParameter>>> OutliersAccum(m_MaxIterations);
...
#pragma omp parallel for
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				...
				auto EvalPair = RandomModel->EvaluateWithOutliers(RemainderSamples, m_Threshold);

				InlierFractionAccum[i] = std::get<0>(EvalPair);
				InliersAccum[i] = std::get<1>(EvalPair);
				OutliersAccum[i] = std::get<2>(EvalPair);
			...
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				if (InlierFractionAccum[i] > m_BestModelScore)
				{
					...
					m_BestOutliers = OutliersAccum[i];
