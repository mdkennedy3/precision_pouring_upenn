...
public:
  ...
  virtual std::tuple<GRANSAC::VPFloat, std::vector<std::shared_ptr<GRANSAC::AbstractParameter>>, std::vector<std::shared_ptr<GRANSAC::AbstractParameter>>> EvaluateWithOutliers(const std::vector<std::shared_ptr<GRANSAC::AbstractParameter>> &EvaluateParams, GRANSAC::VPFloat Threshold)
    {
  std::vector<std::shared_ptr<GRANSAC::AbstractParameter>> Inliers, Outliers;
  int nTotalParams = EvaluateParams.size();
  int nInliers = 0;
  ...
  return std::make_tuple(InlierFraction, Inliers, Outliers);
    };
